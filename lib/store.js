...
export async function updateBusinessSettings(branchId, settings) {
  if (!isSupabaseMode()) {
    return localStore.updateBusinessSettings(branchId, settings);
  }

  const payload = await apiRequest(`/api/branches/${branchId}/settings`, {
    method: "PATCH",
    body: settings,
  });
  return payload.branches || [];
