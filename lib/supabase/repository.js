...
function normalizeBusinessSettingsInput(input, branch) {
...
function mapBranch(row) {
  const settingsRow = Array.isArray(row.branch_business_settings)
    ? row.branch_business_settings[0]
    : row.branch_business_settings;
  return {
    id: row.id,
    name: row.name,
    address: row.address,
    storeName: settingsRow?.store_name || row.name,
    receiptFooter: settingsRow?.receipt_footer || DEFAULT_RECEIPT_FOOTER,
    whatsapp: settingsRow?.whatsapp || "",
    operatingHours: settingsRow?.operating_hours || "",
    enabledPaymentMethods: sanitizePaymentMethods(settingsRow?.enabled_payment_methods),
  };
}
...
export async function ensureSeedData() {
  ...
  const { data: existingSettings, error: existingSettingsError } = await client
    .from("branch_business_settings")
    .select("branch_id");
  if (existingSettingsError) {
    throw new Error(existingSettingsError.message);
  }

  const existingSettingBranchIds = new Set(
    (existingSettings || []).map((setting) => setting.branch_id),
  );
  const missingDefaultSettings = DEFAULT_BRANCHES.filter(
    (branch) => !existingSettingBranchIds.has(branch.id),
  ).map((branch) => ({
    branch_id: branch.id,
    ...
    whatsapp: "",
    operating_hours: "",
    enabled_payment_methods: DEFAULT_PAYMENT_METHODS,
  }));

  if (missingDefaultSettings.length) {
    const { error: settingsSeedError } = await client
      .from("branch_business_settings")
      .insert(missingDefaultSettings);
    if (settingsSeedError) {
      throw new Error(settingsSeedError.message);
    }
  }
...
export async function createBranchRecord(input) {
  ...
  const { error: settingsError } = await client.from("branch_business_settings").insert({
...
export async function updateBusinessSettingsRecord(branchId, updates) {
  await ensureSeedData();
  const client = getSupabaseAdminClient();
  ...
  const normalizedInput = normalizeBusinessSettingsInput(updates, branch);
  const { error } = await client.from("branch_business_settings").upsert(
    {
      branch_id: branchId,
      store_name: normalizedInput.storeName,
      receipt_footer: normalizedInput.receiptFooter || DEFAULT_RECEIPT_FOOTER,
      whatsapp: normalizedInput.whatsapp,
      operating_hours: normalizedInput.operatingHours,
      enabled_payment_methods: normalizedInput.enabledPaymentMethods,
    },
...
export async function createOrderRecord(orderData) {
  ...
  const client = getSupabaseAdminClient();
  const { data: branchSettings, error: branchSettingsError } = await client
    .from("branch_business_settings")
    .select("enabled_payment_methods")
    .eq("branch_id", orderData.branchId)
    .maybeSingle();
  if (branchSettingsError) {
    throw new Error(branchSettingsError.message);
  }
  ...
  const paymentMethod = ALLOWED_PAYMENT_METHODS.has(orderData.paymentMethod)
    ...
    : "cash";
  const enabledPaymentMethods = sanitizePaymentMethods(branchSettings?.enabled_payment_methods);
