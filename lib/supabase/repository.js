...
export async function ensureSeedData() {
  ...
  const { error: branchSyncError } = await client
    .from("branches")
    .upsert(DEFAULT_BRANCHES, { onConflict: "id" });
  if (branchSyncError) {
    throw new Error(branchSyncError.message);
  }
  ...
  const { error: userSyncError } = await client
    .from("app_users")
    .upsert(seededUsers, { onConflict: "id" });
  if (userSyncError) {
    throw new Error(userSyncError.message);
  }
